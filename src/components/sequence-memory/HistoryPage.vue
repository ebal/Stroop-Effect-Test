<template>
  <div class="history">
    ...
    <template v-else>
      <p class="trend-note">Longest Sequence trend</p>
      ...
      <div class="entry-list">
        <div v-for="(entry, i) in reversedEntries" :key="i" class="entry-row">
          ...
          <span class="entry-sequence">{{ entry.longestSequence }} cells</span>
          <span class="entry-meta">Lvl {{ entry.highestLevel }} · {{ entry.mistakes }}✕</span>
        </div>
      </div>
    </template>
...
const { getHistory } = useMemoryStats()
...
const sparklinePoints = computed(() => {
  const values = entries.value.map((e) => e.longestSequence)
