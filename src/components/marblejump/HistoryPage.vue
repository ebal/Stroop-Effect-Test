<template>
  <div class="history">
    ...
    <div v-if="entries.length === 0" class="empty">
      No completed games yet{{ activeFilter !== 'all' ? ` for ${activeFilter}` : '' }}.
    </div>

    <template v-else>
      <p class="trend-note">Marbles left trend (lower is better)</p>
      ...
      <div class="entry-list">
        <div v-for="(entry, i) in reversedEntries" :key="i" class="entry-row">
          ...
          <span class="entry-remaining">{{ entry.remainingMarbles }} left</span>
          <span class="entry-meta">
            {{ entry.moves }} moves · {{ formatTime(entry.completionTime) }}
            <span v-if="entry.optimalReached" class="clean-badge optimal-badge">optimal</span>
            <span v-else-if="entry.clean" class="clean-badge">clean</span>
          </span>
        </div>
      </div>
    </template>
...
const { getHistory } = useMarbleJumpStats()
...
const sparklinePoints = computed(() => {
  const values = entries.value.map((e) => e.remainingMarbles)
  if (values.length === 0) return ''
  const max = Math.max(...values)
  const min = Math.min(...values)
  const range = max - min || 1
  const step = values.length > 1 ? 100 / (values.length - 1) : 0
  return values.map((v, i) => `${i * step},${4 + ((v - min) / range) * 32}`).join(' ')
})
