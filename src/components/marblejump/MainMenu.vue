<template>
  <div class="menu">
    <h1>Marble Jump</h1>
    <p class="subtitle">Jump over marbles and remove them. Plan ahead and leave as few as possible.</p>
    ...
    <div class="difficulty-grid">
      <button v-for="d in difficulties" :key="d.key" class="difficulty-card" @click="handleStart(d.key)">
        <h2>{{ d.label }}</h2>
        <p class="meta">{{ d.holes }}-hole board</p>
        <div class="stat-line" v-if="stats(d.key).completed > 0">
          Best left: {{ stats(d.key).bestResult ? stats(d.key).bestResult.remaining : '—' }}
        </div>
...
const difficulties = Object.values(MARBLEJUMP_DIFFICULTIES)
const { getActive, clearActive } = useMarbleJumpStorage()
const { getDerivedStats, recordAbandon } = useMarbleJumpStats()
