<template>
  <div class="history">
    ...
    <div v-if="entries.length === 0" class="empty">
      No completed puzzles yet{{ activeFilter !== 'all' ? ` for ${activeFilter}` : '' }}.
    </div>
    ...
    <template v-else>
      ...
      <div class="entry-list">
        <div v-for="(entry, i) in reversedEntries" :key="i" class="entry-row">
          ...
          <span class="entry-time">{{ formatTime(entry.completionTime) }}</span>
          <span class="entry-meta">
            {{ entry.mistakes }}✕ {{ entry.hints }}💡
            <span v-if="entry.cleanSolve" class="clean-badge">clean</span>
          </span>
        </div>
      </div>
    </template>
...
const { getHistory } = useSudokuStats()
...
const sparklinePoints = computed(() => {
  const times = entries.value.map((e) => e.completionTime)
  if (times.length === 0) return ''
  const max = Math.max(...times)
  const min = Math.min(...times)
  const range = max - min || 1
  const step = times.length > 1 ? 100 / (times.length - 1) : 0
  return times.map((t, i) => `${i * step},${4 + ((t - min) / range) * 32}`).join(' ')
})
