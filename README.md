...
A timed spatial-reasoning game: see a reference shape and 2-4 candidates, tap the one that's the
reference *rotated* — as fast and accurately as you can before the round timer runs out. Mirrored
or structurally different candidates are always wrong, even when they look close at a glance.
...
- Continuous round: answer, get brief ✓/✕ feedback, next trial, until time's up. Backgrounding the
  app pauses the timer and discards the in-progress trial rather than scoring a stale one.
- Score rewards accuracy and speed together, but Accuracy and Median Correct RT are always shown
  separately as the more meaningful raw measurements.
- Untimed, unscored Practice mode included.
