...
- **Docker (revised — no custom image)**: two services, both plain official images, no Dockerfile:
  - `stroop` — unmodified `nginx:alpine`, bind-mounts `./dist` read-only into `/usr/share/nginx/html`. Always-on; `docker compose up -d` / `stop` / `start` never build anything, so they're instant.
  - `builder` — unmodified `node:20-alpine`, bind-mounts the whole project, runs `npm install && npm run build` to produce `./dist`. Not started by `docker compose up` (it's under the `build` profile) — run it explicitly only when the source changes: `docker compose --profile build run --rm builder`. Because `dist/` is bind-mounted straight into the running nginx container, there's not even a restart needed afterward — refreshing the browser picks up the new files immediately.
- **docker-compose.yml**: exposes port `8888:80` on the `stroop` service, `restart: unless-stopped`. No backend database; app state (best scores, history) lives in the browser via `localStorage`, so the containers themselves stay fully stateless.
...
3. Exposed port: **8888** (host) → 80 (container, nginx).
...
- **Removed the custom Dockerfile entirely** — replaced with two plain-official-image Compose services (see §5): `stroop` (nginx:alpine, bind-mounted `dist/`, always instant to start/stop) and `builder` (node:20-alpine, run on demand only when source changes, under the `build` profile). Motivation: `docker compose up/start/stop` should never require a build, and the running service should be a completely unmodified upstream image.
